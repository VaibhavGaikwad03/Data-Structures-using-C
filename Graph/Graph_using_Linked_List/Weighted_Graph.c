...
void WeightedGraph(struct vertex **ppHead)
{
    ...
    while(pTravelVertex != NULL)
    {
        ...
        for(iCnt1 = 1; iCnt1 <= iNoAdjVertices; iCnt1++)
        {
            ...
            printf("Enter weight for %d --> %d : ", pTravelVertex->iData, iAdjVertex);
            scanf("%d", &pNewEdge->iWeight);
    ...
    while(pTravelVertex != NULL)
    {
        ...
        {
            printf("%d(%d) ", pTravelEdge->pAdjVertex->iData, pTravelEdge->iWeight);
            pTravelEdge = pTravelEdge->pNextAdjEdge;
        }
        printf("\n");
        pTravelVertex = pTravelVertex->pNextVertex;
